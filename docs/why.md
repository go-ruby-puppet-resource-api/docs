...
`go-ruby-puppet-resource-api/puppet-resource-api` reimplements Ruby's `puppet-resource-api` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-puppet-resource-api/puppet-resource-api` directly, with no Ruby runtime;
...
Because `puppet-resource-api` is pure Go with cgo disabled, it also compiles to **WebAssembly**
